...
- **专业签署**:党史/军史专家(史实)、作家/文学编辑(文字)、画家/美术编辑(画面)
  只签署自己专业范围内的意见;编辑负责采纳、驳回与状态流转。意见签署时绑定目标
  版本,并冻结该页面版本采用的脚本/设定/史料修订快照。采纳前再次核对当前版本:
  目标版本已过期的意见只能留档,或由同专业专家用 `countersign_opinion` 转签到
  当前版本后重新走采纳流程,不能直接改变新稿状态。
- **意见留档**:意见状态为 提出 → 采纳/驳回,采纳后可撤回;曾被采纳的意见
  (含已撤回)任何人不得删除。
- **冲突会审**:同一对象、**同一目标版本**、同一专业范围内结论冲突的已采纳意见
  才自动生成会审议题,页面转入联合会审;重大事实议题未关闭不得转入精稿。意见
  撤回/驳回使同版同范围失去对立时,议题自动「已消解」;页面换版后,旧页面版本
  议题与新稿不再采用的旧依赖版本议题「随版留档」,均不再阻断新稿。学员可对
  当前版本的未结/已关闭议题提交附史料依据的异议(关闭的议题重开,已进入
  精稿/可出版的页面退回联合会审);已消解或随旧版留档的议题不能凭异议复活,
  须针对新版本重新签署。
- **授权检查**:素材授权期、保密期、出版范围随页面版本在交付与导出时检查;
  过期授权、事实争议或依赖陈旧的页面明确保持不可交付。
- **批量导出**:`export_batch` 在开始时冻结候选页面版本、依赖最新版本、史料
  许可与未结议题;处理开始后才产生的新版本、新画格、新许可、新议题一律不混入
  本次导出。许可按本次导出日期(`on`)与出版范围(`scope`)判断,被排除页面
  返回 `{page_version, reasons}`,每条原因标注其对应的页面版本。
- **全程追溯**:`panel_trace` 从任一画格反查采用的史料、文字版本、
  采纳/撤回的意见与决定人,并带出版本绑定与议题状态。
...
`withdraw_opinion`、`countersign_opinion`、`delete_opinion`、`close_issue`、
`submit_objection`、`transition_page`、`page_blockers`、`panel_trace`、
`export_batch`。

`page_blockers` 与 `export_batch` 支持 `on`(ISO 日期,默认今天);导出排除项
为 `{page_version, reasons}`,原因中带「(页面vN)」版本标注。
